              ...
              {!error && (!loading.loadingGetById ? (
                <div className=" flex  flex-col items-start gap-1 dark:text-white">
                  ...
                  <div className="flex  items-start gap-1">
                    ...
                    <span className={student?.phoneParent ? "" : "text-gray-500"}>
                      {student?.phoneParent ? student?.phoneParent : "Aucune Télé Parent"}
                    </span>
                  </div>
                  <div className="flex  items-start gap-1">
                    <span className="font-semibold text-gray-400 flex-shrink-0">
                  ...
                  <div className="flex  items-start gap-1">
                    ...
                    <span className={student?.centre?.name ? "" : "text-gray-500"}>
                      {student?.centre?.name ? student?.centre?.name : "Aucune Centre"}
                    </span>
                  </div>
                  <div className="flex  items-start gap-1">
                    <span className="font-semibold text-gray-400 flex-shrink-0">Niveau :</span>
                    <span className={student?.level?.name ? "" : "text-gray-500"}>
                      {student?.level?.name ? student?.level?.name : "Aucune Niveau"}
                    </span>
                  </div>
                  <div className="flex items-start gap-1">
                    <span className="font-semibold text-gray-400 flex-shrink-0">
                    ...
                    <span className="flex flex-col">
                      {student?.subjects && student.subjects.length > 0 ? (
                        student.subjects.map((subject) => (
                          <span className="font-semibold flex gap-1" key={subject.id}>
                            - {subject.name} : <span className="underline">{subject.pricePerMonth} DH</span>
                          </span>
                        ))
                      ) : (
